...
const findTradesSchema = zod_1.z.object({
    userId: zod_1.z.string(),
    albumId: zod_1.z.string().optional(),
    radiusKm: zod_1.z.number().optional(),
});
const proposeTradeSchema = zod_1.z.object({
    proposerId: zod_1.z.string(),
    receiverId: zod_1.z.string(),
    items: zod_1.z.array(zod_1.z.object({
        figurinhaId: zod_1.z.string(),
        direction: zod_1.z.enum(['OFFER', 'REQUEST']),
    })),
});
...
async function tradeRoutes(app) {
    ...
    app.post('/find', async (request, reply) => {
        ...
        const myInventory = await index_js_1.prisma.inventory.findMany({
        ...
        const userFilter = { id: { not: userId } };
        if (albumId) {
            // Only figurinhas from this album
        }
        // Find other users who have what I want (my faltantes)
        const othersWithMyWants = await index_js_1.prisma.inventory.findMany({
            where: {
                userId: { not: userId },
                status: 'REPETIDA',
        ...
        const othersWantMyRepetidas = await index_js_1.prisma.inventory.findMany({
            where: {
                userId: { not: userId },
                status: 'FALTANTE',
        ...
        for (const item of othersWithMyWants) {
            const existing = otherUsersMap.get(item.user.id) || {};
            if (!existing.theyHave)
                existing.theyHave = [];
        ...
        for (const [, data] of otherUsersMap) {
            const theyHave = data.theyHave || [];
            const theyWant = data.theyWant || [];
            if (theyHave.length === 0 && theyWant.length === 0)
                continue;
            let distanceKm;
            const myUser = await index_js_1.prisma.user.findUnique({ where: { id: userId }, select: { lat: true, lng: true } });
        ...
        matches.sort((a, b) => {
            if (a.type === 'PERFEITA' && b.type !== 'PERFEITA')
                return -1;
            if (b.type === 'PERFEITA' && a.type !== 'PERFEITA')
                return 1;
    ...
    app.post('/propose', async (request, reply) => {
        ...
        const trade = await index_js_1.prisma.trade.create({
            data: {
                proposerId,
                receiverId,
                tradeItems: {
    ...
    app.post('/:id/accept', async (request, reply) => {
        ...
        const trade = await index_js_1.prisma.trade.findUnique({ where: { id } });
        if (!trade)
            return reply.status(404).send({ error: 'Trade not found' });
        if (trade.receiverId !== payload.id)
            return reply.status(403).send({ error: 'Not your trade' });
        if (trade.status !== 'PENDENTE')
            return reply.status(400).send({ error: 'Trade not pending' });
        // Execute the trade: update inventory
        const tradeItems = await index_js_1.prisma.tradeItem.findMany({ where: { tradeId: id } });
        for (const item of tradeItems) {
            if (item.direction === 'OFFER') {
                // Giver loses 1
                await index_js_1.prisma.inventory.updateMany({
        ...
        const updated = await index_js_1.prisma.trade.update({
            where: { id },
            data: { status: 'ACEITA' },
        });
    ...
    app.get('/history', async (request, reply) => {
        ...
        const payload = request.user;
        const trades = await index_js_1.prisma.trade.findMany({
